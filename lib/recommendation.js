...
//
// Phase 2 (active now that the data files below are populated): counters,
// ally synergy, and real win/pick/ban rate, sourced from Liquipedia's
// Honor of Kings World Cup 2026 Statistics page (pro tournament data, not
// solo-queue ladder — see source caveat below). Split across three files
// by concern, since each is likely to get updated independently (a new
// tournament refreshes stats; counters/synergy are closer to patch notes):
//
//   data/hero-tournament-stats.json — aggregate numbers per hero:
//     { "<slug>": { winRate, gamesPlayed, pickRate, banRate, source } }
//
//   data/hero-counters.json — matchup relationships:
//     { "<slug>": { counters: ["slug", ...], counteredBy: ["slug", ...] } }
//     counters = heroes THIS hero has a strong (>=55% WR, >=3 games) record against
//     counteredBy = heroes with a strong record against THIS hero
//
//   data/hero-synergy.json — team-comp relationships:
//     { "<slug>": { synergizesWith: ["slug", ...] } }
//     synergizesWith = ally heroes with a strong (>=55% WR, >=3 games) record alongside this one
//
// A hero missing from any of the three, or missing individual fields, just
...
// way — the >=3-game floor applied when these files were built already
// screens out 1-game flukes there.

import { heroBySlug } from "./heroes";
import tournamentStatsData from "@/data/hero-tournament-stats.json";
import counterData from "@/data/hero-counters.json";
import synergyData from "@/data/hero-synergy.json";
...
// applied when these files were built already filters out 1-game flukes.
...
function getTournamentStats(slug) {
  return tournamentStatsData[slug] || null;
}

function getCounterData(slug) {
  return counterData[slug] || null;
}

function getSynergyData(slug) {
  return synergyData[slug] || null;
}
...
function heroCounters(hero, opponentSlug) {
  const c = getCounterData(hero.slug);
  if (c?.counters?.includes(opponentSlug)) return true;
  const oppC = getCounterData(opponentSlug);
  if (oppC?.counteredBy?.includes(hero.slug)) return true;
  return false;
}

function heroIsCounteredBy(hero, opponentSlug) {
  const c = getCounterData(hero.slug);
  if (c?.counteredBy?.includes(opponentSlug)) return true;
  const oppC = getCounterData(opponentSlug);
  if (oppC?.counters?.includes(hero.slug)) return true;
  return false;
}
...
export function scoreBanCandidate(hero, { comfortHeroLevels = {}, algorithmMode = "standard" } = {}) {
  ...
  const stats = getTournamentStats(hero.slug);
...
// teamPicks: your own team's picks so far, as [{ slug, role }] (role is the
// lane that hero was actually drafted for — matters for flex heroes)
...
export function scorePickCandidate(
  ...
  const heroSynergy = getSynergyData(hero.slug);
  const teamPickSlugs = teamPicks.map((p) => p.slug);
  const synergyAllies = teamPickSlugs.filter((slug) => heroSynergy?.synergizesWith?.includes(slug));
...
// The set of lanes your team already has covered — based on the SPECIFIC
// role each hero was drafted to play, not every role that hero could
// theoretically fill. A flex hero like Kongming only occupies whichever
// single lane the player picked it for.
export function getFilledLanes(teamPicks) {
  const set = new Set();
  teamPicks.forEach((p) => {
...
// phase: "ban" | "pick"
// teamPicks: current team's picks so far, as [{ slug, role }] (used for "pick": lane gaps + synergy)
...
export function getSuggestions({
  ...
  if (phase === "pick") {
    ...
    // Don't suggest a hero that has no open lane left to fill. A Mid-only
    // hero once Mid is taken is a dead suggestion; a Mid/Jungle flex hero
    // still counts as long as Jungle is open — it'll just be scored (and
    // reasoned) against that one remaining lane.
    const eligibleHeroes = availableHeroes.filter((hero) =>
      hero.roles.some((r) => !filledLanes.has(r))
    );
