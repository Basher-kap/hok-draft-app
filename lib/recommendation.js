...
// shape (see BALANCE_TEMPLATES / ACTIVE_BALANCE_TEMPLATES in
// lib/heroArchetypes.js). Four standard shapes are defined there, but for
// now the engine is FIXED to just the first one:
//   2 Tank / 2 Damage(L+S) / 1 Semi-Tank   <- active target
//   2 Semi-Tank / 2 Damage(L+S) / 1 Semi-Tank Support   (defined, not active)
//   3 Damage(2 Short+1 Long) / 1 Tank / 1 Semi-Tank     (defined, not active)
//   3 Damage(2 Short+1 Long) / 2 Tank                   (defined, not active)
// The target stays "achievable" as long as the team hasn't drafted more of
// any bucket than it calls for. A candidate gets a bonus if it can fill a
// bucket the target still needs, and a WARNING if there's no way to play
// it - in any of its still-open lanes - that keeps the target alive. Once
// the team's comp has already drifted off the target shape, this layer
// just goes quiet rather than keep complaining about a comp that's
// already off-script.
//
...
const BALANCE_FIT_BONUS = 16; // fills a bucket the fixed target comp still needs
const BALANCE_BREAK_PENALTY = 22; // no way to play this hero keeps the fixed target comp alive
...
export function scorePickCandidate(hero, { filledLanes, algorithmMode, comfortLevel, enemyPickEntries, teamPickEntries }) {
  ...
  // from) the fixed target shape (ACTIVE_BALANCE_TEMPLATES - currently
  // just "2 Tank / 2 Damage(L+S) / 1 Semi-Tank")? Only reasoned about
  // while the target is still reachable - once the comp has already
  // drifted off it, this stays quiet instead of nagging about a comp
  // that's already off-script.
  ...
  if (achievableBefore.length > 0) {
    ...
    if (matchedOption) {
      ...
      compositionReasons.push({
        text: `fits the target comp (${humanizeBucket(matchedOption.bucket)} · ${matchedOption.role})`,
        type: "positive",
      });
    }
    ...
    if (distinctBuckets.length > 0 && !survivesSomeOption) {
      ...
      compositionReasons.push({ text: `breaks the target comp (${achievableBefore[0].label})`, type: "warning" });
    }
  }
