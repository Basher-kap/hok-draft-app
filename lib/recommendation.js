...
import { broadCategoriesForHero, heroHasCC, heroIsAntiTank, compositionSummary, achievableTemplates, pickPrimaryTemplate, neededBucketsFor, bucketOptionsForHeroInLanes, humanizeBucket } from "./heroArchetypes";
import { normalizedRankedMetaFor } from "./rankedMeta";
import { normalizedTournamentMetaFor } from "./tournamentMeta";
import { comfortTierFor, COMFORT_TIER_LABELS, COMFORT_TIER_ORDER } from "./comfortHeroes";
...
const TEMPLATE_FIT_BONUS = 18; // this hero fills a bucket a still-achievable balance template needs
const TEMPLATE_BREAK_PENALTY = 22; // drafting this hero, in ANY of its open lanes, would leave zero achievable balance templates
...
// Ban-phase threat signals - shared between Rank Draft and Tournament
// Draft's ban scorers, and independent of which meta data source is in
// play. A "just ban the highest tier/ban-rate hero" model misses two
// things that actually matter at the table:
//   1. a hero who'd slot straight into a synergy combo with what the
//      enemy has already picked - leaving it up denies them that combo
//   2. a hero who specifically counters what WE'VE already picked - a
//      live, concrete threat to our own lineup, not just a generically
//      strong hero
const BAN_SYNERGY_DENY_BONUS_PER_HIT = 22; // candidate would synergize with an enemy pick if left available
const BAN_COUNTER_DEFENSE_BONUS_PER_HIT = 25; // candidate counters one of OUR picks specifically

function banThreatSignals(hero, { enemyPickEntries, teamPickEntries } = {}) {
  let bonus = 0;
  const reasons = [];

  const partners = synergyPartnersFor(hero.slug);
  const enemySlugs = (enemyPickEntries || []).map((p) => p.slug);
  const enemySynergyHits = enemySlugs.filter((slug) => partners.has(slug));
  if (enemySynergyHits.length > 0) {
    bonus += enemySynergyHits.length * BAN_SYNERGY_DENY_BONUS_PER_HIT;
    const names = enemySynergyHits.map((s) => heroBySlug(s)?.name || s).join(", ");
    reasons.push({ text: `would synergize with their ${names}`, type: "positive" });
  }

  const { strongAgainst } = counterDataFor(hero.slug);
  const teamSlugs = (teamPickEntries || []).map((p) => p.slug);
  const threatensUs = teamSlugs.filter((slug) => strongAgainst.includes(slug));
  if (threatensUs.length > 0) {
    bonus += threatensUs.length * BAN_COUNTER_DEFENSE_BONUS_PER_HIT;
    const names = threatensUs.map((s) => heroBySlug(s)?.name || s).join(", ");
    reasons.push({ text: `counters your ${names}`, type: "positive" });
  }
...
export function scoreRankedBanCandidate(hero, { enemyPickEntries, teamPickEntries } = {}) {
  ...
  const { bonus: threatBonus, reasons: threatReasons } = banThreatSignals(hero, { enemyPickEntries, teamPickEntries });
  reasons.push(...threatReasons);

  return { hero, score: tierScore + rankedBonus + threatBonus, reasons };
}
...
export function scoreTournamentBanCandidate(hero, { enemyPickEntries, teamPickEntries } = {}) {
  const tierScore = TIER_SCORE[hero.tier] ?? 0;
  ...
  return { hero, score: tierScore + tournamentBonus + threatBonus, reasons };
}
...
export function scorePickCandidate(hero, { filledLanes, algorithmMode, comfortLevel, getComfortTier, enemyPickEntries, teamPickEntries, mode = "tournament" }) {
  ...
  // Balance-template fit / break (lib/heroArchetypes.js BALANCE_TEMPLATES)
  // - the four standard 5-hero composition shapes, narrowed down by this
  // team's picks so far. Pure bucket-count math (achievableTemplates),
  // same as the composition bonuses above - doesn't need the full
  // remaining hero pool the way feasibleTemplates does.
  const achievable = achievableTemplates(teamComp);
  if (achievable.length > 0) {
    const primaryTemplate = pickPrimaryTemplate(teamComp);
    const neededBuckets = neededBucketsFor(teamComp, achievable);
    const candidateBucketOptions = bucketOptionsForHeroInLanes(hero.slug, missingCovered);

    // Bonus: this hero can fill a bucket the current target template (or
    // any still-achievable template) still needs.
    const fitOption = candidateBucketOptions.find((o) => neededBuckets.has(o.bucket));
    if (fitOption) {
      compositionBonus += TEMPLATE_FIT_BONUS;
      compositionReasons.push({
        text: `completes the ${humanizeBucket(fitOption.bucket)} slot in your ${primaryTemplate.label} shape (${fitOption.role})`,
        type: "positive",
      });
    }

    // Warning: no matter which open lane this hero is drafted into, the
    // resulting comp would match ZERO of the currently-achievable
    // templates - i.e. it overflows every one of them. Only fires when
    // the hero actually has lane options to check (a hero with no
    // archetype data for any open lane can't be judged either way).
    if (candidateBucketOptions.length > 0) {
      const breaksEveryOption = candidateBucketOptions.every((o) => {
        const nextComp = { ...teamComp, [o.bucket]: (teamComp[o.bucket] || 0) + 1 };
        return achievableTemplates(nextComp).length === 0;
      });
      if (breaksEveryOption) {
        compositionBonus -= TEMPLATE_BREAK_PENALTY;
        compositionReasons.push({ text: "would leave your comp off every standard balance shape", type: "warning" });
      }
    }
  }
  ...
  if (comfortTierBonus > 0) {
    const label = COMFORT_TIER_LABELS[comfortTierHit.tier];
    reasons.push({
      text: comfortTierOffCurve
        ? `${label} comfort pick (${comfortTierHit.lane}) \u2014 off pick-curve`
        : `${label} comfort pick (${comfortTierHit.lane})`,
      type: "positive",
    });
  }
  if (beats.length > 0) {
...
// phase: "ban" | "pick"
// teamPickEntries: current team's picks so far, as [{slug, role}] - used
//   in ban phase for counter-defense ("would this hero counter one of
//   OUR picks"), in pick phase for lane-gap/comfort-tier/synergy
// enemyPickEntries: opposing team's picks so far, as [{slug, role}] - used
//   in ban phase for lane-denial exclusion and synergy-denial bonus, in
//   pick phase for counter bonus
...
export function getSuggestions({
  ...
  if (phase === "ban") {
    ...
    // Hard exclusion: a hero whose ENTIRE role list is already covered by
    // the enemy's own picks has no denial value - the enemy has nowhere
    // left to use it even if it stays available, so banning it wastes a
    // ban. A flex hero who also plays a lane the enemy still has open
    // stays eligible normally (it still denies THAT lane).
    const enemyFilledLanes = getFilledLanes(enemyPickEntries);
    const worthDenying = availableHeroes.filter((hero) => hero.roles.some((r) => !enemyFilledLanes.has(r)));
    return worthDenying
      .map((hero) => banScorer(hero, { enemyPickEntries, teamPickEntries }))
