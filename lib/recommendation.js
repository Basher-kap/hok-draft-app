...
//
// On top of that, every pick is checked against a target "balance lineup"
// shape (see BALANCE_TEMPLATES in lib/heroArchetypes.js). Four standard
// shapes are defined:
...
// The engine doesn't commit to one of these up front - it dynamically
// narrows the set as the draft goes:
//   1. achievableTemplates(teamComp) drops any shape where the team has
//      already drafted MORE of a bucket than that shape calls for (pure
//      bucket-count math on this team's own picks).
//   2. feasibleTemplates() narrows further using the REAL remaining hero
//      pool - a shape only survives if every bucket it still needs can
//      actually be filled by some hero not yet banned or picked by
//      EITHER side, in one of this team's open lanes. So a shape can drop
//      out purely because the enemy banned or picked the last hero who
//      could have filled it - this is where bans feed into the picture,
//      not just this team's own picks.
// A candidate gets a bonus if it can fill a bucket some still-feasible
// shape needs, and a WARNING if there's no way to play it - in any of its
// still-open lanes - that keeps even one feasible shape alive. Once every
// shape has been ruled out, this layer goes quiet rather than keep
// complaining about a comp that's already off-script by that point.
//
...
import { heroBySlug, counterDataFor, communityCounterNote, synergyPartnersFor, ROLES } from "./heroes";
import {
  compositionSummary,
...
const BALANCE_FIT_BONUS = 16; // fills a bucket some still-feasible balance shape needs
const BALANCE_BREAK_PENALTY = 22; // no way to play this hero keeps ANY feasible shape alive
...
export function scorePickCandidate(
  hero,
  { filledLanes, algorithmMode, comfortLevel, enemyPickEntries, teamPickEntries, feasibleTemplatesForTeam }
) {
  const tierScore = TIER_SCORE[hero.tier] ?? 0;
  ...
  // from) one of the standard shapes still realistically reachable given
  // the current hero pool (feasibleTemplatesForTeam, hoisted once per
  // getSuggestions call - see there for how bans factor in)? Only
  // reasoned about while at least one shape is still feasible - once
  // every shape's been ruled out, this stays quiet instead of nagging
  // about a comp that's already off-script.
  const achievableBefore = feasibleTemplatesForTeam || [];
  if (achievableBefore.length > 0) {
    ...
    if (matchedOption) {
      ...
      // Name the specific shape once the field has narrowed to just one -
      // otherwise keep it generic since several shapes still fit equally.
      const text =
        achievableBefore.length === 1
          ? `fits ${achievableBefore[0].label} (${humanizeBucket(matchedOption.bucket)} · ${matchedOption.role})`
          : `fits a balanced comp (${humanizeBucket(matchedOption.bucket)} · ${matchedOption.role})`;
      compositionReasons.push({ text, type: "positive" });
    }

    const distinctBuckets = [...new Set(bucketOptions.map((o) => o.bucket))];
    // Simulated against `achievableBefore` (already narrowed to feasible
    // shapes), not the full template set - feasibility only ever gets
    // MORE restrictive as heroes get drafted, so a shape already ruled
    // out here can't come back after this pick.
    const survivesSomeOption = distinctBuckets.some((b) => {
      const simulated = { ...teamComp, [b]: (teamComp[b] || 0) + 1 };
      return achievableBefore.some((t) => COMPOSITION_BUCKETS.every((bucket) => (simulated[bucket] || 0) <= t[bucket]));
    });
    if (distinctBuckets.length > 0 && !survivesSomeOption) {
      compositionBonus -= BALANCE_BREAK_PENALTY;
      const text =
        achievableBefore.length === 1
          ? `breaks the only fitting comp (${achievableBefore[0].label})`
          : "doesn't fit any standard balanced 5-hero comp";
      compositionReasons.push({ text, type: "warning" });
    }
  }
...
export function getSuggestions({
  ...
  if (phase === "pick") {
    ...
    // Which standard balance shapes are still realistically reachable for
    // THIS team, given both this team's own picks AND the shared hero pool
    // (which already reflects everyone's bans and picks)? Computed once
    // per call, not per candidate - see feasibleTemplates in
    // lib/heroArchetypes.js for how bans factor in.
    const teamComp = compositionSummary(teamPickEntries);
    const openLanes = ROLES.filter((r) => !filledLanes.has(r));
    const feasibleTemplatesForTeam = feasibleTemplates(teamComp, openLanes, availableHeroes);
