...
// Plain tier + lane-flexibility ban score, with no meta-stats blended
// in. Kept as a reference/building-block function - scoreRankedBanCandidate
// and scoreTournamentBanCandidate both start from this same base and add
...
// available. Distinct from scoreTournamentBanCandidate on purpose: that
// one bans off pro/tournament-scene relevance, this one bans off what's
...
export function scoreTournamentBanCandidate(hero) {
  ...
  return { hero, score: tierScore + flexBonus + tournamentBonus, reasons };
}

export function scorePickCandidate(hero, { filledLanes, algorithmMode, comfortLevel, enemyPickEntries, teamPickEntries, mode = "tournament" }) {
  const tierScore = TIER_SCORE[hero.tier] ?? 0;
  ...
  // Ranked-ladder / pro-scene meta (win rate + pick rate only - ban rate
  // isn't a pick-relevance signal). Mirrors the ban-phase scorers: same
  // data sources (lib/rankedMeta.js / lib/tournamentMeta.js), same 0-100
  // normalization, selected by `mode`. A hero with no meta entry yet
  // simply gets no bonus - pick scoring is unchanged from before this
  // existed until real data is populated for it.
  ...
  return {
    hero,
    score: tierScore + gapBonus + comfortBonus + counterBonus + synergyBonus + compositionBonus + metaBonus,
    reasons,
  };
}
