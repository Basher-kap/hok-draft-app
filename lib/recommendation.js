...
// Default comfort-hero tiering (lib/comfortHeroes.js, data/comfort-heroes.json)
// - a separate, always-on signal from the manual super/comfort assignments
// above. It doesn't require the "comfort" algorithm toggle and isn't
// user-edited; it's Basher's own fixed pick-priority read per lane.
// Weighted heavily in Tournament Draft (where this model is meant to
// matter most) and present but lighter in Rank Draft, where real ladder
// meta (win/pick/ban rate) is already the dominant signal.
...
export function scorePickCandidate(hero, { filledLanes, algorithmMode, comfortLevel, getComfortTier, enemyPickEntries, teamPickEntries, mode = "tournament" }) {
  ...
  // Default comfort-tier bonus (independent of the manual toggle above).
  // Checked against every lane this hero could still fill - if it's a
  // comfort tier in more than one open lane, the best (highest-priority)
  // tier wins. `getComfortTier` lets a caller inject the user's own edited
  // tier assignments (components/ComfortTierProvider.jsx); falls back to
  // the shipped default data (lib/comfortHeroes.js) when not provided.
...
//   "adjust" | "wildcard" - the default comfort-tier model (pick phase
//   only). Pass useComfortTier()'s `tierFor` here to use the user's edited
//   assignments; omitted, scorePickCandidate falls back to the shipped
//   default data (lib/comfortHeroes.js).
