...
import { rankedMetaFor } from "./rankedMeta";
...
// Rank Draft-only ban weights - see lib/rankedMeta.js. These are blended
// straight onto the tier + flex-lane score below (not a replacement for
// it) so a hero with no ranked meta data yet still bans exactly like it
// did before this existed. Once real numbers are populated, ranked meta
// intentionally outweighs tier alone - "what's actually getting banned/
// winning/picked on ladder right now" is a stronger signal than a static
// tier badge for THIS mode specifically.
const RANKED_BAN_RATE_WEIGHT = 1.5;
const RANKED_WIN_RATE_WEIGHT = 1.2;
const RANKED_PICK_RATE_WEIGHT = 1.0;
...
// Rank Draft's ban-phase scorer - blends tier + lane-flexibility (same as
// scoreBanCandidate) with real ranked-ladder ban/win/pick rate, when
// available. Distinct from scoreBanCandidate on purpose: Tournament Draft
// bans off pro/tournament-scene relevance, Rank Draft bans off what's
// actually dominant on the ranked ladder right now - the two metas can
// and do diverge.
export function scoreRankedBanCandidate(hero) {
  ...
  const meta = rankedMetaFor(hero.slug);
  let rankedBonus = 0;
  if (meta) {
    rankedBonus =
    ...
    reasons.push({
      text: `${meta.banRate}% ban rate · ${meta.winRate}% win rate · ${meta.pickRate}% pick rate`,
      type: "positive",
    });
  }

  return { hero, score: tierScore + flexBonus + rankedBonus, reasons };
}
...
// mode: "tournament" (default) | "rank" - selects which ban-phase scorer
//   to use. Rank Draft passes "rank" to get ranked-ladder meta (ban/win/
//   pick rate) blended in; Tournament Draft leaves this as the default,
//   which keeps its existing tier + flex-lane ban scoring. Pick-phase
//   scoring is unaffected either way.
export function getSuggestions({
  ...
  if (phase === "ban") {
    const banScorer = mode === "rank" ? scoreRankedBanCandidate : scoreBanCandidate;
