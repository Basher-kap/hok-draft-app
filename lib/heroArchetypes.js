...
// complete 5-hero team's worth of composition buckets. All four are kept
// defined here for reference, but only ACTIVE_BALANCE_TEMPLATES (below)
// is actually used by the suggestion engine right now.
...
// FIXED FOR NOW: the suggestion engine drafts toward exactly ONE shape -
// "2 Tank / 2 Damage (L+S) / 1 Semi-Tank" - not whichever of the four
// happens to still be reachable. To go back to "any of the four standard
// shapes", change this to `BALANCE_TEMPLATES` (all four).
export const ACTIVE_BALANCE_TEMPLATES = [BALANCE_TEMPLATES[0]];

// A template is still reachable if the team hasn't already drafted MORE of
// any bucket than that template calls for. Checks against
// ACTIVE_BALANCE_TEMPLATES, not the full set of four - see above.
