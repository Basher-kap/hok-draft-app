...
// Team A always leads PICK_ORDER (see array above), so Team A is Blue
// Side / first pick and Team B is Red Side. This is a naming layer only -
// it doesn't change the state machine, just labels what's already true
// structurally. Snake pick order means:
//   Blue (A): picks 1, 4, 5, 8, 9   - opens blind, then gets two grouped
//             "double picks" (4+5, 8+9) after seeing Red's replies.
//   Red  (B): picks 2, 3, 6, 7, 10  - always reacts to at least one Blue
//             pick before choosing, and gets the very last, best-informed
//             pick of the draft (10).
// Only pick 1 (index 0) is truly blind - every other pick has at least
// one enemy hero on the board to read.
export const SIDE_META = {
  A: { label: "Blue Side", tag: "First Pick", color: "#3b82f6" },
  B: { label: "Red Side", tag: "Counter Pick", color: "#ef4444" },
};

// Strategic role of a given pick-phase step, purely from its index in
// PICK_ORDER - doesn't need draft state.
//   "blind"    - no enemy picks exist yet (only ever step 0 / Blue's P1)
//   "reactive" - at least one enemy hero is already on the board
export function pickContext(pickIndex) {
  return pickIndex === 0 ? "blind" : "reactive";
}
...
export function initialDraftState() {
  return {
    ...
    picks: { A: [], B: [] }, // arrays of { slug, role } - role is the LANE this pick was drafted for
  };
}

// role is required for pick-phase actions (the lane this hero is being
// played as, chosen by the person drafting - a flex hero locks in one
// lane per pick). Ignored for ban-phase actions.
...
export function isHeroPicked(state, slug) {
  return state.picks.A.some((p) => p.slug === slug) || state.picks.B.some((p) => p.slug === slug);
}
