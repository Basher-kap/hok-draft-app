...
//   Phase 1 - Bans:  3 total
//   Phase 2 - Picks: 5 per team (10 total)
//
// Assumption (flagged for confirmation): bans alternate A/B/A, and picks
// follow a snake order so no team picks back-to-back unfairly. Both are
// easy to change here in one place if the real HoK ranked order differs.

export const BAN_ORDER = ["A", "B", "A"];
...
export function isHeroTaken(state, heroSlug) {
  return (
    ...
    state.picks.A.includes(heroSlug) ||
    state.picks.B.includes(heroSlug)
  );
}
